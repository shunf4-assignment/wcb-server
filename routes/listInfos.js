...
module.exports = function (req, res, next) {
    ...
    var findPromise = model.Info.find(query, {supplementary: 0}, {skip: page * itemPerPage, limit: itemPerPage, sort: {updateTime: -1}}).exec()
        .then(function(infoDocs){
            result.infos = infoDocs;
        }).catch(function(err){
            throw {httpCode: 500, result: {succeed: false, msg: "查询出错"}, error: err};
        });

    var countPromise = model.Info.count(query).exec()
        .then(function(count) {
            result.total = count;
        }).catch(function(err){
            throw {httpCode: 500, result: {succeed: false, msg: "查询出错"}, error: err};
    ...
    Promise.all([userPromise, findPromise, countPromise]).then(function(){
        ...
        console.error("Error at listInfos", err.error);
        if (err.httpCode) {
