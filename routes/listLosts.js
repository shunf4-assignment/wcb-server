...
module.exports = function (req, res, next) {
    ...
    var findPromise = model.LostNFound.find(query, {supplementary: 0}, {skip: page * itemPerPage, limit: itemPerPage, sort: {lastUpdated: -1}}).exec()
        .then(function(lnfDocs){
            result.losts = lnfDocs;
        }).catch(function(err){
            throw {httpCode: 500, result: {succeed: false, msg: "查询出错"}, error: err};
        });

    var countPromise = model.LostNFound.count(query).exec()
        .then(function(count) {
            result.total = count;
        }).catch(function(err){
            throw {httpCode: 500, result: {succeed: false, msg: "查询出错"}, error: err};
    ...
    Promise.all([userPromise, findPromise, countPromise]).then(function(){
        ...
        console.error("Error at listLosts", err.error);
        if (err.httpCode) {
